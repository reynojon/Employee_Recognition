...
module.exports.postSignature = function(req, res, next) {
    ...
    if (req.session.userId && req.session.userType == 'E') {
        ...
        client.query("UPDATE signature SET data =($1) WHERE e_id = ($2);", [data.filename, req.session.userId], function(err, result) {
            if (err) {
                return next(err);
            }
            //res.send(200);
...
module.exports.businessQueries = function(req, res) {
    //res.render('index',{err:"something went terribly wrong!"});
    res.render('businessQueries');
};